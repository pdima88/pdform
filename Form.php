...
class Form
{
    ...
    function render()
    {
        if (!isset($this->renderer)) {
            ...
            if (class_exists('S4Y_Form_Renderer_'.ucfirst($renderer))) {
                $className = 'S4Y_Form_Renderer_'.ucfirst($renderer);
    ...
    function toArray($ignoreFields = []) {
        ...
        foreach ($this->fields as $name => &$f) {
            ...
            if (isset($this->fields[$name]['ignore']) &&
                $this->fields[$name]['ignore']) continue;
            $arr[$name] = $this->getValue($name);
            if ($this->fields[$name]['type'] == 'date' ||
                $this->fields[$name]['type'] == 'dateInput') {
                if (!empty($arr[$name])) {
        ...
        foreach ($this->fields as $name => &$element) {
            ...
            if ($element['type'] === 'imgFile' ||
                ...
                if ($isValidFile && $element['type'] === 'imgFile') {
                    S4Y_Form_Field_ImgFile::validate($name, $this);
                }
            }
